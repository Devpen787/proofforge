...
The web prototype must also follow the product-experience contract in
[`UX_JOURNEY_RULES.md`](./UX_JOURNEY_RULES.md). That document was the original
gate for navigation, screen jobs, card density, primary CTAs, and first-time
user clarity.

Hackathon bounty claims must follow
[`BOUNTY_ALIGNMENT.md`](./BOUNTY_ALIGNMENT.md). That document separates
implemented integrations from planned integrations so the submission stays
defensible.

Execution order for those integrations is tracked in
